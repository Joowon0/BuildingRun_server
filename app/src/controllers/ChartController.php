...
final class ChartController extends BaseController {
  ...
  public function minAirQuality($MAC, $startDate, $endDate) {
    $sql = "SELECT SUBSTR(Timestamp, 1, 16) as TIME, avg(CO) AS CO, avg(SO2) AS SO2, avg(NO2) AS NO2, avg(O3) AS O3, avg(PM25) AS PM25, avg(TEMP) as TEMP FROM AirQuality_Info WHERE MAC = '".$MAC."' AND '".$startDate."' <= Timestamp  AND Timestamp < '".$endDate."' GROUP BY SUBSTR(Timestamp, 1, 16)";
    try {
    	$stmt = $this->db->query($sql);
    	$result = $stmt->fetch();
  ...
  public function hourAirQuality($MAC, $startDate, $endDate) {
    $sql = "SELECT SUBSTR(Timestamp, 1, 13) as TIME, avg(CO) AS CO, avg(SO2) AS SO2, avg(NO2) AS NO2, avg(O3) AS O3, avg(PM25) AS PM25, avg(TEMP) as TEMP FROM AirQuality_Info WHERE MAC = '".$MAC."' AND '".$startDate."' <= Timestamp  AND Timestamp < '".$endDate."' GROUP BY SUBSTR(Timestamp, 1, 13)";
    try {
    	$stmt = $this->db->query($sql);
    	$result = $stmt->fetch();
...
GROUP BY SUBSTR(Timestamp, 1, 10) LIMIT 10";
  ...
  public function app_heartHistory(Request $request, Response $response, $args) {
    ...
    if (isset($jsonArray['USN']) && isset($jsonArray['period']) &&
      ...
      if ($jsonArray['period'] == 1)
        $airData = $this->minHeart($jsonArray['USN'], $jsonArray['startDate'], $jsonArray['endDate']);
      else if ($jsonArray['period'] == 2)
  ...
  public function minHeart($USN, $startDate, $endDate) {
    $sql = "SELECT SUBSTR(Timestamp, 1, 16) as TIME, avg(HeartRate) as HeartRate, avg(HeartInterval) as HeartInterval
            FROM Heart_Info
            WHERE USN = ".$USN." AND
            '".$startDate."' <= Timestamp  AND Timestamp < '".$endDate."' GROUP BY SUBSTR(Timestamp, 1, 16)";
    try {
    	$stmt = $this->db->query($sql);
    	$result = $stmt->fetch();
